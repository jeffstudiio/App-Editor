...
## ۶ب) بستهٔ «Editor UX & Timeline Rebuild» (۱۴۰۴/۰۶/۲۱) — انجام شد

| مورد مشخصات | وضعیت |
|---|---|
| P0-1/2 Workspace + پیش‌نمایش همیشه‌مرئی | ✅ ToolDock داخلی ≤46dvh (موبایل) + Inspector (دسکتاپ lg+) — Sheet 88dvh حذف شد |
| P0-3 سیستم انتخاب (کلیپ/متن/صدا/لایه/**ترنزیشن**) | ✅ + ابزار بافتاری کامل |
| P0-4/5/6 برش واقعی + مدل Edit-Point + اتصال ترنزیشن به مرز | ✅ `splitClipAt` + `Project.transitions` + cleanup داخل mutate |
| P0-7 نوار ابزار بافتاری | ✅ کلیپ/ترنزیشن/متن/صدا/لایه هرکدام مجموعهٔ خودشان |
| P1-8 مرورگر ترنزیشن با پیش‌نمایش واقعی | ✅ پیش‌نمایش زندهٔ موتور دوم روی کلیپ‌های کاربر + ۱۷ کارت واقعی |
| P1-9 مرورگر قالب | ✅ از قبل واقعی (بانک ۸۷)؛ ترنزیشن قالب‌ها حالا مرزی واقعی |
| P1-10/11 مرورگر پریست و افکت | ✅ PresetsBrowser + EffectsBrowser (فقط ماژول واقعی) |
| P1-12 موبایل بهتر | ✅ ToolDock + زوم پینچ + اهداف لمسی ≥44px در نوار ابزار |
| P1-13 Undo/Redo | ✅ از قبل واقعی؛ حالا ترنزیشن‌ها هم تحت آن + تست §44.8 |
| §44 هشت تست رگرسیون | ✅ scripts/test-editor-regressions.ts — ۵۳ assert سبز |
| P2-14..18 (موشن پیشرفته، speed curve، ماسک پیشرفته، graph editor، کتابخانه‌های بزرگ‌تر) | ⏳ باقی — به Sprint بعد |

فاصله‌های باقی‌مانده از این بسته: وایپ با لبهٔ نرم (feather)، ترنزیشن بین‌ترکی (لایه↔اصلی)،
درگ‌دستی مستقیم روی پیش‌نمایش (move/scale با handle)، Speed curve ویرایش‌پذیر، InShot-style
long-press preview. این‌ها P2 اعلام شدند و در «مقایسه با محصول هدف» پایین لحاظ شده‌اند.
